    ...
    if request.method == 'POST':
        ...
        if bus:
            if bus.rem > int(seats_r):
                ...
                book = Book.objects.create(name=username_r, flight_no=flight_r , email=email_r, userid=userid_r, bus_name=name_r,
                                           source=source_r, busid=id_r,
                                           dest=dest_r, price=price_r, nos=seats_r, date=date_r, time=time_r,
                                           date1=date_r1, time1=time_r1 ,status='BOOKED')
